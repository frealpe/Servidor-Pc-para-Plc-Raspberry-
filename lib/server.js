...
class Server {
    ...
    async startOpcReading() {
        ...
        const callback = (valor) => {
            ...
            // Publicar en MQTT
            // mqttClient.publish("Plc/Resultado", valor.toString(), { qos: 0 }, (err) => {
    ...
    async conectarDB() {
        ...
        console.log('Base de datos conectada');
           ...
           mqttClient.on('message', (topic, message) => {
               ...
               console.log('Recibido Plc/Respuesta:', msg);

         //       Ejemplo: reenviar a WebSocket
            //     if (this.wss) {
            //         this.wss.clients.forEach((client) => {
            //         if (client.readyState === 1) {
            //             client.send(JSON.stringify({ respuestaPLC: msg }));
            //         }
            //         });
            //     }
                }
